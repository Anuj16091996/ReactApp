...
class Playlist extends React.Component {
    ...
    componentDidMount() {
        fetch("https://localhost:8000/track").then((response) => {
    ...
    DeleteData = (id) => {
        ...
        let url = "https://localhost:8000/track/" + id;
    ...
    AddDataToPlayList = (id) => {
        ...
        fetch("https://localhost:8000/track", {
            method: "post",
            headers: {
    ...
    SearchDataThroughAPI = () => {
        ...
        fetch("https://localhost:8000/playlist").then((response) => {
            if (response.ok) {
                response.json().then((json_response) => {
                    this.setState({
            ...
            return (
                <tbody key={index}>
                    <td>
                        ...
                        ID: {data.id}
                        <br />
                        Playlist :{data.playlist_title}
                        <br />
                        <a href={data.uri}> {data.uri} </a>
                    </td>

                    <td>
                        <button onClick={() => this.DeleteData(data.id)}>
                            Delete{" "}
                        </button>
                    </td>
                </tbody>
            ...
            return (
                <tbody key={index}>
                    <td>
                        ...
                        <a href={data.uri}> {data.uri} </a>
                        <br />
                        <a href={"http://www.discogs.com" + data.uri}>
                            {" "}
                            More Information{" "}
                        </a>
                    </td>
                    ...
                    <td>
                        <button onClick={() => this.AddDataToPlayList(index)}>
                            {" "}
                            Add{" "}
                        </button>
                    </td>
            ...
            <div>
                ...
                <div className="DisplayContent">
                    <div className="Playlist">
                        <h4>My Playlist</h4>
                        <div className="">
                            <table>{dataBaseArrayItems}</table>
                            {console.log(dataBaseArrayItems)}
                        </div>
                    </div>
                    ...
                    <div className="API">
                        ...
                        <pre>
                            ...
                            <b> Candian releases only </b>
                        </pre>

                        <button onClick={() => this.SearchDataThroughAPI()}>
                            Search{" "}
                        </button>
                        <br />

                        <div>{discordArrayItems}</div>
                    </div>
                </div>
            </div>
        );
